...
//console.log("Cargada funcion: \t Leer showCar()");
 
		function showCar(date_salida, date_retorno, time_salida, time_retorno) {
			...
			document.getElementById("lista_autos").innerHTML = this.responseText;
			}
			console.log("Ejecutada");
			};
			xmlhttp.open("GET","php/autos.php?date_salida="+date_salida+"&date_retorno="+date_retorno+"&time_salida="+time_salida+"&time_retorno="+time_retorno,true);
			xmlhttp.send();
			}
		}
		...
		$( ("input[name=f_salida]") && ("input[name=f_retorno]") && ("input[name=h_salida]")&&("input[name=h_retorno]")).change(function(){
			...
			showCar(date_salida ,date_retorno, time_salida, time_retorno);
		});
