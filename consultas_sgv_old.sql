...
/*
	Carga lista de ejes
*/
SELECT * FROM hubs ORDER BY caption;
...
*/
SELECT id, `name` FROM users WHERE NOT deleted ORDER BY `name`;
...
/*
	Carga de programas relativos a un eje
	:HUBID eje especificado
*/
SELECT * FROM programs WHERE hubid = :HUBID;
...
/*
	Carga la lista de municipios relativos al estado seleccionado
	:STATE id del estado seleccionado
*/
SELECT id, `name` FROM municipalities WHERE state = :STATE ORDER BY `name`;
...
INSERT INTO `log` VALUES (NULL, 1, :RECORD, :USER, 2, '', NOW());
